...
def loadCam(args, id, cam_info, resolution_scale, flow_dirs):
    ...
    resized_image_gray = cam_info.image.convert('L')
    resized_image_gray = PILtoTorch(resized_image_gray, resolution)
    ...
    cam = Camera(colmap_id=cam_info.uid, R=cam_info.R, T=cam_info.T,
                  ...
                  depth=depth, mono_depth=mono_depth, gt_image_gray=gt_image_gray, flow_dirs=flow_dirs)
    return cam
...
def gen_virtul_cam(cam, trans_noise=1.0, deg_noise=15.0):
    ...
    virtul_cam = Camera(100000, Rt[:3, :3].transpose(), Rt[:3, 3], cam.FoVx, cam.FoVy, image=cam.image,
                        gt_alpha_mask= None, image_name = cam.image_name, uid= 100000,
